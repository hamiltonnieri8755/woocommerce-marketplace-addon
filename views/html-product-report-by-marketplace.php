<?php
/**
 * Admin View: Product Report by Marketplace
 */
	...
	<div class="postbox">
		<h3 class="stats_range">
			...
			<div class="item-input-wrapper">
				<div id="item_id"></div>
				<input type="button" class="button" id="set_id" value="Show">
			</div>
			<ul>
				<li id="year" class="tab-link"><a href="?page=wc-reports&tab=orders_mp&report=sales_by_product&range=year&item_id=<?php echo $this->item_id; ?>">Year</a></li>
				<li id="lastmonth" class="tab-link"><a href="?page=wc-reports&tab=orders_mp&report=sales_by_product&range=lastmonth&item_id=<?php echo $this->item_id; ?>">Last Month</a></li>
				<li id="thismonth" class="tab-link"><a href="?page=wc-reports&tab=orders_mp&report=sales_by_product&range=thismonth&item_id=<?php echo $this->item_id; ?>">This Month</a></li>
				<li id="last7day" class="tab-link"><a href="?page=wc-reports&tab=orders_mp&report=sales_by_product&range=last7day&item_id=<?php echo $this->item_id; ?>">Last 7 Days</a></li>				
				<li id="custom" class="custom">
					Custom:					
					<form method="GET">
						<div>
							<input name="item_id" value="<?php echo $_GET['item_id']; ?>" type="hidden">
							...
							<input name="report" value="sales_by_product" type="hidden">							
				...
				<h2>
					<?php
						if ( ! empty($this->item_id) ) {
							echo get_post_meta( $this->item_id, "_sku", true) . " - " . get_post( $this->item_id )->post_title . " is selected.";
						} else {
				...
				<ul class="chart-legend">
					...
					<li style="border-color: <?php echo $this->chart_colours[0]; ?>" class="highlight_series" data-series="0">
						...
						<div class="details-wrapper">
							<?php
								echo "<span>" . 'Purchases for the selected items : ' . $this->get_products_count_mp(0, "product") . "</span><br/>";
							?>
						</div>
					</li>
					...
					<li style="border-color: <?php echo $this->chart_colours[1]; ?>" class="highlight_series" data-series="1">
						...
						<div class="details-wrapper">
							<?php
								echo "<span>" . 'Purchases for the selected items : ' . $this->get_products_count_mp(1, "product") . "</span><br/>";
							?>
						</div>
					</li>
					...
					<li style="border-color: <?php echo $this->chart_colours[2]; ?>" class="highlight_series" data-series="2">
						...
						<div class="details-wrapper">
							<?php
								echo "<span>" . 'Purchases for the selected items : ' . $this->get_products_count_mp(2, "product") . "</span><br/>";
							?>
						</div>
					</li>
					...
					<li style="border-color: <?php echo $this->chart_colours[3]; ?>" class="highlight_overall">
						...
						<div class="details-wrapper">
							<?php
								echo "<span>" . 'Purchases for the selected items : ' . $this->get_products_count_mp(3, "product") . "</span><br/>";
							?>
						</div>
					</li>
...
<script type="text/javascript">
	jQuery(document).ready(function (){
		...
		// init and focus on item keyword input box
		jQuery("#item_id").select2({
			allowClear:  false,
		...
		jQuery("#item_id").change(function () {
			jQuery("#item_id_custom").val(jQuery("#item_id").val());
		});

		jQuery("#set_id").click(function () {
			window.location.href = window.location.href + "&item_id=" + jQuery("#item_id").val();
		});
